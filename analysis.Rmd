...
The worked examples use `jeffreys_pseudo_belief_function()` below.
The scaling experiment uses the same `jeffreys_interval()` calculation,
with a subset-sum transform to aggregate counts efficiently.
...
Direct LP calculations for the examples with $|\Omega|=3,4$ are very fast.
We therefore use generated data to see how the sparse LP formulation behaves
for $|\Omega|=5,\ldots,14$. This is an illustration, not a general benchmark.
...
For each data set, choose $k$ uniformly from $1,\ldots,|\Omega|$ and sample
$k$ distinct nonsingleton subsets. Together with all singletons, these are
the observed subsets. Give each one observation first. Distribute the remaining
$200-(|\Omega|+k)$ observations multinomially using normalized Gamma$(1,1)$
weights. Consequently every selected subset has a positive count.
...
Set `run_scaling_experiment` to TRUE and run this chunk.
For each $|\Omega|=5,\ldots,10$ there are 20 data sets; for
$|\Omega|=11,12,13,14$ there are five. The time limit is 300 seconds per LP
for every frame size. None of the five calculations for
$|\Omega|=14$ finished within this limit, and larger frames were not attempted.
...
and save one result row. Elapsed times are measured with `system.time()`.
For a simple memory indication, `gc(reset=TRUE)` resets R's memory counter
before each data set; the reported value is the subsequent increase in maximum
memory managed by R. This deliberately simple measure depends on when garbage
collection occurs, whether R reuses previously allocated memory, and one-time
allocations in the first runs. It also excludes memory allocated internally by
HiGHS. Fluctuations between runs should therefore not be overinterpreted; the
...
The final chunk groups result rows by $|\Omega|$. Each numerical cell contains
the arithmetic mean followed by the maximum in square brackets.
...
calculation time and R-managed memory grow with $|\Omega|$. A failed allocation
or a solver time limit is not evidence that the feasible polytope is empty.
