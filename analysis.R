#' ---
#' title: 'From Confidence Bounds to Belief Functions: Experimental Supplement'
...
#' 
#' These objectives encode different preferences. L1 minimizes the total increase
#' of the belief values above the input bounds and is therefore a data-fidelity
#' criterion. HD maximizes expected log-cardinality and directly favours
#' nonspecificity. CW also favours larger focal elements, but uses inverse
#' cardinality rather than logarithmic weights. Consequently, the three LPs may
#' select different optimal faces, although ties or coincident solutions are also
#' possible.
#' 
...
#' 
#' We now repeat the same calculation on randomly generated data. The steps are
#' unchanged: obtain Jeffreys lower bounds, construct a dominating belief function,
#' and compare L1, HD and CW. We reuse the functions already introduced:
#' the interval calculation, the three objective functions, and the small-example
#' routines for LP/UAP and vertex enumeration.
#' 
#' For educational clarity, the small-frame examples use a straightforward
#' dense-matrix formulation with the R package `lpSolve`. For larger frames,
#' we use HiGHS, an open-source optimization solver accessed through the R
#' package `highs`, which supports large sparse constraint matrices.
#' The data provide a lower bound for each subset of the frame, so the number
#' of potential dominance constraints grows exponentially with the frame size.
#' Their coefficients are nonzero only when one subset is contained in another,
#' making sparse storage particularly useful here.
#' 
...
#' focal sets. Assign normalized Gamma(1,1) masses and draw 200 observations.
#' The counts determine the Jeffreys bounds at alpha = 0.05; as before,
#' the empty-set and full-frame bounds are fixed to 0 and 1.
#' 
...
#' the constraint for A follows from that for B and monotonicity. It is enough
#' to check immediate predecessors. The empty set is excluded from this test:
#' a nonempty zero-count singleton has a positive Jeffreys lower bound.
#' 
#' The objective weights below come from the same L1, HD and CW functions used
#' in the small examples. Only subset cardinalities are needed.
#' 
## ----sparse-constraints-------------------------------------------------------
...
#' objective. Each solution is checked against **all** lower bounds, including
#' the rows removed above, as well as nonnegativity, normalization and zero
#' empty-set mass. We retain times, focal counts and the largest constraint
#' violation. A temporary checkpoint allows partial results to survive if a
#' large calculation has to be stopped.
#' 
## ----sparse-solver------------------------------------------------------------
...
#' 
#' A measurement uses a fresh R process, so memory from earlier calculations is
#' not carried over. Solver time covers the three LP calls; total computation
#' time also includes preparation and validation. Process startup and package
#' loading are excluded from these computation times.
#' 
#' Peak RSS measures resident process memory, including R, loaded packages and
#' native solver allocations. It is not the size of the R objects alone.
#' On Windows we also record peak committed memory and use the operating
#' system's peak counters. Elsewhere RSS is sampled and is a lower bound on
#' the true peak. All memory values are in MiB (2^20 bytes).
#' 
...
#' 
#' <details>
#' <summary>Implementation of the measurement helper</summary>
#' 
#' This helper is independent of the belief-function calculation. It loads only
#' the functions needed by a child process and returns the result with memory
#' measurements. It can be read separately from the experiment itself.
#' 
...
#' 
#' For n = 5,...,10, we use 20 independently seeded inputs per size. For
#' n = 11,...,15 and 20, we use one input per size and a 180-second limit per
#' LP. These larger cases are exploratory probes, not a universal dimension
#' limit. Success or failure also depends on the generated counts.
#' 
#' The small examples reuse compute_table() and compute_vertex_minima(), with
#' five repetitions per task. The same process-level measurement is used for
#' all experiments.
#' 
...
#' memory peaks. Failed or guarded runs are not included in successful-solve
#' time and focal-count summaries. Their observed memory remains reported.
#' A missing three-LP time means that the full calculation did not finish;
#' the memory observed before a stop is not the memory required to finish.
#' 
## ----summary-functions--------------------------------------------------------
...
#' 
#' The measured tables below are included directly in this document.
#' They use R 4.3.2, lpSolve 5.6.23, rcdd 1.6 and highs 1.9.0-1 on Windows.
#' Times and memory depend on the computer and its load. Fixed seeds reproduce
#' the inputs; a different solver version can choose another point of the
#' same optimal face.
#' 
#' Set run_benchmarks to TRUE in the execution block below to replace these
#' tables with fresh measurements. The additional packages for this step are
#' highs, Matrix, callr and ps. The small worked examples need only the packages
#' loaded earlier.
#' 
#' <details>
#' <summary>Recorded measurements used for the displayed tables</summary>
#' 
## ----recorded-results---------------------------------------------------------
recorded <- list(
...
#' </details>
#' 
#' Before a full rerun, we check that the sparse and dense LPs give the same
#' objective values on small inputs. This also checks the subset-sum transform
#' and the special treatment of the empty set. Different mass vectors can be
#' equally optimal.
#' 
#' <details>
#' <summary>Small-case consistency checks</summary>
#' 
## ----validate-sparse-r--------------------------------------------------------
...
#' 
#' In the recorded run, all 120 scaling instances were solved and validated.
#' Three different mass vectors were obtained in 117 cases; in three cases
#' some objectives selected the same vector. The larger probes completed through
#' n = 13. The n = 14 and n = 15 probes reached the process-memory guard.
#' For n = 20, the estimated storage of one dominance matrix alone was
#' 7.93 GiB, so the matrix was not allocated.
#' 
#' These observations concern the recorded inputs and resource limits, not an
#' intrinsic limit on frame size. In particular, a guarded run does not imply
#' that a dominating belief function does not exist. On rerunning the experiment,
#' read the outcome columns rather than assuming the same stopping points.
#' 
## ----session-information, results='asis'--------------------------------------
cat("```text\n",
