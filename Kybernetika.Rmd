...
UAP in a single summary table (**Table 9, A+LP**).
...
knitr::kable(
  table,
  caption = "Table 1 (modified) — UAP succeeds for the adjusted pseudo-belief function g̃.",
  align = "lrrrr",
  row.names = FALSE
)
```
...
knitr::kable(
  tab4,
  caption = "Table 4 — UAP failure for Jeffreys lower bounds with α = 0.05.",
  align = "lrrrr",
  row.names = FALSE
)
```
...
knitr::kable(
  tab5,
  caption = "Table 5 — UAP succeeds for Jeffreys lower bounds with α = 0.02.",
  align = "lrrrr",
  row.names = FALSE
)
```
...
```{r}
## Table 6 — Discounting δ = 0.02 applied to Jeffreys g from Table 4

delta <- 0.02

# Start from g of Table 4 (α = 0.05)
g4_vals <- g4_res$g
names(g4_vals) <- g4_res$subset_names
...
knitr::kable(
  tab6,
  caption = "Table 6 — UAP succeeds after Shafer discounting with δ = 0.02.",
  align = "lrrrr",
  row.names = FALSE
)
```
...
solve_lp_for_objective <- function(Omega, g_values, obj_fun) {
  ...
  # Reconstruct belief f(A) = Σ_{B ⊆ A} m(B)
